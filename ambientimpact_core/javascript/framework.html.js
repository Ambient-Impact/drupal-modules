...
AmbientImpact.onGlobals([
	...
	// Pass the delay Promise to the framework for the components that have
	// HTML.
	...
	new Promise(function(resolve, reject) {
		...
		$.get(settings.html.endpointPath).done(function(data) {
			...
			// When the response from the server has come in, resolve the
			// component delay Promise.
			resolve();
		});
	}));
	 ...
	 *
	 * Note that this doesn't need to be aware of the Ajax request or use any
	 * sort of Promise as components only get added/registered when the Ajax
	 * request succeeds.
	 *
	 * @return {string}
