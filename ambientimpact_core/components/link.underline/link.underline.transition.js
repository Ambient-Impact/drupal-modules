...
AmbientImpact.onGlobals([
	...
	// only registers its component if CSS custom properties are supported, so
	// we don't need to check for support - we simply won't get this far.
	'TweenLite.to',
], function() {
...
AmbientImpact.addComponent('link.underline.transition', function(
	...
	var
		// This is the custom property that we programmatically fade out, as
		// it's used in the CSS for the underline.
		...
		// The name of the data key to save the current link's TweenLite
		// instance to.
	...
	// given link element. These are set in the CSS, in
	// @mixin fancy-link-underlines.
	...
	function linkEnterHandler(event) {
		...
		// Remove the current colour property so that the active state colour
	...
	function linkLeaveHandler(event) {
		...
		// If this element is still the currently focused element, don't fade
		// out. This can happen if the user clicks and drags the link, in which
		// case 'mouseleave' will fire but the element is still focused and will
		// fire 'blur'. This is to prevent the underline from fading from the
		// active state until it actually should.
		...
		// This doesn't work, but the enter handler clears this anyways, so
		// leave this hear for now in case GSAP starts removing it as it should.
	...
	this.addBehaviors({
		AmbientImpactLinkUnderlineTransition: {
			attach: function(context, settings) {
				$(context)
					// Bind event handlers. Since we're using named event
					// handlers, we probably don't need to use $().once()?
