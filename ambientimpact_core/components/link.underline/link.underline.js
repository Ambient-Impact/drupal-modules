...
AmbientImpact.addComponent('link.underline', function(aiLinkUnderline, $) {
	...
	// available, so that the child components still load if custom properties
	// are supported.
	if (
		!window.getSelection ||
	...
	// This is added to links when the window is not focused so that we can
	// remove underlines if they are part of the selection. This is done because
	// the inactive selection pseudoclasses are not well supported yet.
