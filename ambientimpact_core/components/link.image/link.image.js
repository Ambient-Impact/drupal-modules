...
AmbientImpact.addComponent('link.image', function(aiLinkImage, $) {
    ...
    wrapImageLinkText: function() {
      if (aiLinkImage.isImageContainingLink(this)) {
        $(this)
          ...
          // Get all text nodes.
          .contents().textNodes()
            // Filter out any text nodes that are already wrapped, and any text
            // nodes that are within elements we're not supposed to wrap
            // within.
            .filter(function() {
              return (
                $(this).parents(
  ...
  // extension as external, opening in a new tab and not attempting to load via
  // Ajax pages.
